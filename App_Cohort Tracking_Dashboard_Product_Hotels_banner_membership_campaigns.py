# App Cohort Tracking Dashboard - Product Hotels banner & membership campaigns
...
monthly_view_query = """ 
 WITH cohort_agg_device AS (   
  ...
  FROM user_scratch.x_kmahendra.membership_campaign_combined_sep_01
  --ANALYTICS.ACIES.CTA_LTV_ALL_CAMPAIGNS
  WHERE 
    --CAMPAIGN_DASH_STATUS = True
    --AND TOTAL_CAMPAIGN_COST > 0 AND TOTAL_CAMPAIGN_COST IS NOT NULL
    --AND CTA_CAMPAIGN IN ('Product_Hotels Banner 1')
    CTA_NAME = {select_cta_name}
...
FROM 
user_scratch.x_kmahendra.membership_campaign_combined_sep_01
  --ANALYTICS.ACIES.CTA_LTV_ALL_CAMPAIGNS
--WHERE 
    --TOTAL_CAMPAIGN_COST > 0 AND TOTAL_CAMPAIGN_COST IS NOT NULL
    -- and CAMPAIGN_DASH_STATUS = 'true'
    -- CTA_CAMPAIGN IN ('Product_Hotels Banner 1')
;
"""
...
WITH member_percent_cte AS (
  ...
  FROM ANALYTICS.ACIES.CTA_LTV_ALL_CAMPAIGNS
...
FROM 
user_scratch.x_kmahendra.membership_campaign_combined_sep_01 main
  --ANALYTICS.ACIES.CTA_LTV_ALL_CAMPAIGNS 
...
SELECT 
    ...
    (CASE WHEN SUM(OBSERVED_LTV_15D) IS NOT NULL THEN SUM(OBSERVED_LTV_15D) ELSE SUM(EXTRAPOLATED_LTV_15D) END) / NULLIF(SUM(CTA_QUALIFYING_ACTION_COUNT),0) AS LTV_15D,
    CASE WHEN SUM(OBSERVED_LTV_15D) IS NOT NULL THEN 'OBSERVED' ELSE 'EXTRAPOLATED' END AS LTV_15D_BASIS,

    (CASE WHEN SUM(OBSERVED_LTV_30D) IS NOT NULL THEN SUM(OBSERVED_LTV_30D) ELSE SUM(EXTRAPOLATED_LTV_30D) END) / NULLIF(SUM(CTA_QUALIFYING_ACTION_COUNT),0) AS LTV_30D,
    CASE WHEN SUM(OBSERVED_LTV_30D) IS NOT NULL THEN 'OBSERVED' ELSE 'EXTRAPOLATED' END AS LTV_30D_BASIS,

    (CASE WHEN SUM(OBSERVED_LTV_45D) IS NOT NULL THEN SUM(OBSERVED_LTV_45D) ELSE SUM(EXTRAPOLATED_LTV_45D) END) / NULLIF(SUM(CTA_QUALIFYING_ACTION_COUNT),0) AS LTV_45D,
    CASE WHEN SUM(OBSERVED_LTV_45D) IS NOT NULL THEN 'OBSERVED' ELSE 'EXTRAPOLATED' END AS LTV_45D_BASIS,

    (CASE WHEN SUM(OBSERVED_LTV_60D) IS NOT NULL THEN SUM(OBSERVED_LTV_60D) ELSE SUM(EXTRAPOLATED_LTV_60D) END) / NULLIF(SUM(CTA_QUALIFYING_ACTION_COUNT),0) AS LTV_60D,
    CASE WHEN SUM(OBSERVED_LTV_60D) IS NOT NULL THEN 'OBSERVED' ELSE 'EXTRAPOLATED' END AS LTV_60D_BASIS,

    (CASE WHEN SUM(OBSERVED_LTV_03M) IS NOT NULL THEN SUM(OBSERVED_LTV_03M) ELSE SUM(EXTRAPOLATED_LTV_03M) END) / NULLIF(SUM(CTA_QUALIFYING_ACTION_COUNT),0) AS LTV_03M,
    CASE WHEN SUM(OBSERVED_LTV_03M) IS NOT NULL THEN 'OBSERVED' ELSE 'EXTRAPOLATED' END AS LTV_03M_BASIS,

    (CASE WHEN SUM(OBSERVED_LTV_04M) IS NOT NULL THEN SUM(OBSERVED_LTV_04M) ELSE SUM(EXTRAPOLATED_LTV_04M) END) / NULLIF(SUM(CTA_QUALIFYING_ACTION_COUNT),0) AS LTV_04M,
    CASE WHEN SUM(OBSERVED_LTV_04M) IS NOT NULL THEN 'OBSERVED' ELSE 'EXTRAPOLATED' END AS LTV_04M_BASIS,

    (CASE WHEN SUM(OBSERVED_LTV_05M) IS NOT NULL THEN SUM(OBSERVED_LTV_05M) ELSE SUM(EXTRAPOLATED_LTV_05M) END) / NULLIF(SUM(CTA_QUALIFYING_ACTION_COUNT),0) AS LTV_05M,
    CASE WHEN SUM(OBSERVED_LTV_05M) IS NOT NULL THEN 'OBSERVED' ELSE 'EXTRAPOLATED' END AS LTV_05M_BASIS,

    (CASE WHEN SUM(OBSERVED_LTV_06M) IS NOT NULL THEN SUM(OBSERVED_LTV_06M) ELSE SUM(EXTRAPOLATED_LTV_06M) END) / NULLIF(SUM(CTA_QUALIFYING_ACTION_COUNT),0) AS LTV_06M,
    CASE WHEN SUM(OBSERVED_LTV_06M) IS NOT NULL THEN 'OBSERVED' ELSE 'EXTRAPOLATED' END AS LTV_06M_BASIS,

    (CASE WHEN SUM(OBSERVED_LTV_07M) IS NOT NULL THEN SUM(OBSERVED_LTV_07M) ELSE SUM(EXTRAPOLATED_LTV_07M) END) / NULLIF(SUM(CTA_QUALIFYING_ACTION_COUNT),0) AS LTV_07M,
    CASE WHEN SUM(OBSERVED_LTV_07M) IS NOT NULL THEN 'OBSERVED' ELSE 'EXTRAPOLATED' END AS LTV_07M_BASIS,

    (CASE WHEN SUM(OBSERVED_LTV_08M) IS NOT NULL THEN SUM(OBSERVED_LTV_08M) ELSE SUM(EXTRAPOLATED_LTV_08M) END) / NULLIF(SUM(CTA_QUALIFYING_ACTION_COUNT),0) AS LTV_08M,
    CASE WHEN SUM(OBSERVED_LTV_08M) IS NOT NULL THEN 'OBSERVED' ELSE 'EXTRAPOLATED' END AS LTV_08M_BASIS,

    (CASE WHEN SUM(OBSERVED_LTV_09M) IS NOT NULL THEN SUM(OBSERVED_LTV_09M) ELSE SUM(EXTRAPOLATED_LTV_09M) END) / NULLIF(SUM(CTA_QUALIFYING_ACTION_COUNT),0) AS LTV_09M,
    CASE WHEN SUM(OBSERVED_LTV_09M) IS NOT NULL THEN 'OBSERVED' ELSE 'EXTRAPOLATED' END AS LTV_09M_BASIS,

    (CASE WHEN SUM(OBSERVED_LTV_10M) IS NOT NULL THEN SUM(OBSERVED_LTV_10M) ELSE SUM(EXTRAPOLATED_LTV_10M) END) / NULLIF(SUM(CTA_QUALIFYING_ACTION_COUNT),0) AS LTV_10M,
    CASE WHEN SUM(OBSERVED_LTV_10M) IS NOT NULL THEN 'OBSERVED' ELSE 'EXTRAPOLATED' END AS LTV_10M_BASIS,

    (CASE WHEN SUM(OBSERVED_LTV_11M) IS NOT NULL THEN SUM(OBSERVED_LTV_11M) ELSE SUM(EXTRAPOLATED_LTV_11M) END) / NULLIF(SUM(CTA_QUALIFYING_ACTION_COUNT),0) AS LTV_11M,
    CASE WHEN SUM(OBSERVED_LTV_11M) IS NOT NULL THEN 'OBSERVED' ELSE 'EXTRAPOLATED' END AS LTV_11M_BASIS,

    SUM(EXTRAPOLATED_VALUE_12M) / NULLIF(SUM(CTA_QUALIFYING_ACTION_COUNT),0) AS LTV_12M,
    'EXTRAPOLATED' AS LTV_12M_BASIS
...
def show_bounty_metrics(selected_mva_type, selected_cta_type):
            ...
            if st.button("Submit"):
                ...
                try:
                    ...
                    if included_campaigns:
                        update_true_query = f"""
                        UPDATE user_scratch.x_kmahendra.membership_campaign_combined_sep_01
                            --ANALYTICS.ACIES.CTA_LTV_ALL_CAMPAIGNS
    ...
    with tab_monthly_view:
        ...
        with col2:
            selected_device = st.selectbox(
                ...
                index=sorted(monthly_view_df['DEVICE'].unique()).index("Overall") if "Overall" in monthly_view_df['DEVICE'].unique() else 0
            )

        with col3:
            st.markdown("")  # spacer column
        
        filtered_monthly_df = monthly_view_df[
            (monthly_view_df['Year'].isin(selected_years)) &
            (monthly_view_df['DEVICE'].isin([selected_device]))
        ].drop(columns=['Year'])
        ...
        with channel_col1:
            ...
            default_channel = "googleadwords_int" if "googleadwords_int" in channel_options else (channel_options[0] if channel_options else None)
        ...
        filtered_channel_df = channel_view_df[
            ...
            (channel_view_df['DEVICE'].isin([selected_device]))
        ].drop(columns=['Year'])
    ...
    with tab_all_campaign_view:
        ...
        selected_cta_name = "'app_download_overall'" if selected_mode == "TAMG" else "'app_download_overall_ta_only'"
        all_campaign_query = all_campaign_query_template \
            .replace("{select_cta_name}", selected_cta_name)
        all_campaign_query_df = fetch_data_from_snowflake(all_campaign_query)
        selected_campaigns_all = all_campaign_query_df["CAMPAIGN"].unique()

        # Loop over selected campaigns and render two tables per campaign
        for campaign in selected_campaigns_all:
            st.markdown(f"## 🎯 Campaign: `{campaign}`")
        
            # Filter data for the current campaign and member type = OVERALL        
            all_campaign_df = all_campaign_query_df[
                (all_campaign_query_df["CAMPAIGN"] == campaign)].copy()
            
            # Drop the CAMPAIGN column since it's in the heading
            all_campaign_df.drop(columns=["CAMPAIGN","ON_TRACK_STATUS"], errors='ignore', inplace=True)
        
            # Render On Track Table
            st.markdown("### ✅ Tracking Status")
            if not all_campaign_df.empty:
                st.markdown(render_table_with_colors(all_campaign_df), unsafe_allow_html=True)
            else:
                st.markdown(f"_No data available for **{campaign}**_")
