# WIP App Cohort Tracking Dashboard SKAN (ALL CAMPAIGNS)
...
WITH cohort_agg_device AS (   
  ...
  FROM user_scratch.x_amarium.cta_skan_all_campaigns
  WHERE 
...
available_baseline_query = """
  ...
  --user_scratch.x_akarthick.mva_ltv_new_baseline
...
FROM 
ANALYTICS.ACIES.MVA_COHORT_PERFORMANCE_LATEST
...
FROM 
user_scratch.x_amarium.cta_skan_all_campaigns
--user_scratch.x_amarium.cta_skan_all_campaigns
--USER_SCRATCH.x_jsubramanian.cta_ltv_3_o_A
--USER_SCRATCH.X_AKARTHICK.CTA_LTV_INITIAL
WHERE 
...
SELECT
  ...
  SUM(main.OBSERVED_INCREMENTAL_VALUE) AS OBSERVED_INCREMENTAL_VALUE,
  main.EXPECTED_MVA_INCREMENTAL_VALUE_12M AS EXPECTED_INCREMENTAL_VALUE_12M,
  case when main.EXTRAPOLATED_INCREMENTAL_ROAS <= main.expected_mva_incremental_value_roas_12m
    then ROUND(main.EXTRAPOLATED_INCREMENTAL_ROAS * 100, 2) || '%'
    else
    concat(ROUND((1.2 * OBSERVED_INCREMENTAL_VALUE / nullif(main.total_campaign_cost,0)) * 100,2) || '%'
    , ' - ', ROUND(main.EXTRAPOLATED_INCREMENTAL_ROAS * 100, 2) || '%') 
    end as EXTRAPOLATED_INCREMENTAL_ROAS,
  main.EXTRAPOLATED_INCREMENTAL_12M,
  CASE 
    WHEN main.ON_TRACK_TO_EXCEED_ROAS_TARGET = TRUE THEN 'ON TRACK'
    ELSE 'OFF TRACK'
  END AS ON_TRACK_STATUS

FROM user_scratch.x_amarium.cta_skan_all_campaigns main
...
WHERE 
  ...
  AND main.CTA_DEVICES_COUNT is not null
GROUP BY ALL
ORDER BY 
  CAMPAIGN,
...
query_for_baseline_period_template = """
    ...
    analytics.acies.MVA_LTV
      --user_scratch.x_akarthick.mva_ltv_new_baseline
    WHERE 
       --CURRENT_DATE() BETWEEN VALIDITY_START_DATE AND VALIDITY_END_DATE
      -- AND 
...
SELECT 
  ...
  CTA_DEVICE_GROUP AS DEVICE_GROUP,
  LOWER(MEMBERSHIP_COHORT) AS MEMBERSHIP_COHORT,
  CTA_DEVICES_COUNT AS DEVICES_COUNT,
  CTA_USERS_COUNT AS USERS_COUNT,
  CTA_QUALIFYING_ACTION_COUNT AS QUALIFIED_NEW_INSTALLS,
  CTA_NON_QUALIFYING_ACTION_COUNT AS REINSTALL,
  CTA_ACTIVE_USERS_COUNT AS ACTIVE_USERS,
  (CTA_ACTIVE_USERS_COUNT * 100.0 / NULLIF(CTA_QUALIFYING_ACTION_COUNT, 0)) AS ACTIVE_USERS_PERCENT,
  (CTA_NON_QUALIFYING_ACTION_COUNT * 100.0 / NULLIF(CTA_USERS_COUNT, 0)) AS REINSTALL_PERCENT
FROM 
user_scratch.x_amarium.cta_skan_all_campaigns
--user_scratch.x_amarium.cta_skan_all_campaigns
--USER_SCRATCH.x_jsubramanian.cta_ltv_3_o_A
...
def create_tabs():
    tab_monthly_view,tab_campaign_view,tab_detailed_view,tab_chart, tab_sql,tab_glossary= st.tabs(
            [ "📅 Monthly View","🗃 Campaign View", "💻 Detailed View", "📈 Realization Chart ","🧮 SQL Code","🔤 Glossary"])
    return tab_monthly_view,tab_campaign_view,tab_detailed_view,tab_chart, tab_sql,tab_glossary
...
def show_bounty_metrics(selected_mva_type, selected_cta_type):
            ...
            if st.button("Submit"):
                ...
                try:
                    ...
                    if included_campaigns:
                        update_true_query = f"""
                        UPDATE user_scratch.x_amarium.cta_skan_all_campaigns
    ...
    def sidebar_filters(campaign_meta_query, available_baseline_query, available_performance_cohort_query, fetch_data_from_snowflake):
        ...
        if use_cohort_first:
            ...
            if not selected_campaigns:
                st.warning("Please select at least one Campaign to continue by uncollapsing the side bar")
                st.stop()
            if not selected_cohorts:
    ...
    # Create Tabs
    tab_monthly_view,tab_campaign_view, tab_detailed_view, tab_chart,tab_sql,tab_glossary = create_tabs()
    ...
    filters = sidebar_filters(campaign_meta_query, available_baseline_query, available_performance_cohort_query, fetch_data_from_snowflake)
                       ...
                       for member_type in selected_member_type:
                           if not df.empty:
                               ...
                               if not filtered_df.empty:
                                   ...
                                   if extrapolated_x and extrapolated_y:
                                       if observed_x and observed_y:
                                       ...
                                       fig.add_trace(go.Scatter(
                                           ...
                                           marker=dict(size=6),
                                           text=[f"${format_number(v)}" for v in extrapolated_y],
                                           textposition="top center",
    ...
    with tab_chart:
        ...
        if not any(not df.empty for df in performance_cohort_df):
            ...
            query_for_baseline_period = query_for_baseline_period_template.format(selected_baseline_desc=selected_baseline_desc, locale=selected_locale, device_groups=selected_device_groups,selected_mva_type=selected_mva_type)
                    ...
                    for member_type in selected_member_type:
                        if not df.empty:
                            ...
                            if not filtered_df.empty:
                                ...
                                if extrapolated_x:
                                    if observed_x:
                                        # Insert last observed value to connect line smoothly
                                        extrapolated_x.insert(0, observed_x[-1])
                                        extrapolated_y.insert(0, observed_y[-1])
                                        extrapolated_tooltips.insert(0, observed_tooltips[-1])
...
selected_mva_type = page_names_to_bounty_types[selected_radio_button or "📱 App"]
    ...
    # Call the show_app_bounty_metrics function with the selected bounty type
# with st.spinner(f"Loading data for {selected_radio_button}..."):
show_bounty_metrics(selected_mva_type if not is_tamg else selected_mva_type+"_ta_only", selected_cta_type)
